...
export function detectTechStack(tree: TreeItem[], languages: LanguageBreakdown): TechStackItem[] {
  const stack: TechStackItem[] = [];
  const fileNames = new Set(tree.map((t) => t.path.split("/").pop()?.toLowerCase() || ""));

  if (fileNames.has("package.json")) stack.push({ name: "Node.js", category: "runtime" });
  if (fileNames.has("next.config.js") || fileNames.has("next.config.mjs") || fileNames.has("next.config.ts")) stack.push({ name: "Next.js", category: "framework" });
  if (fileNames.has("nuxt.config.js") || fileNames.has("nuxt.config.ts")) stack.push({ name: "Nuxt.js", category: "framework" });
  if (fileNames.has("vite.config.js") || fileNames.has("vite.config.ts")) stack.push({ name: "Vite", category: "tool" });
  if (fileNames.has("webpack.config.js") || fileNames.has("webpack.config.ts")) stack.push({ name: "Webpack", category: "tool" });
  if (fileNames.has("tailwind.config.js") || fileNames.has("tailwind.config.ts")) stack.push({ name: "Tailwind CSS", category: "library" });
  if (fileNames.has("tsconfig.json")) stack.push({ name: "TypeScript", category: "language" });
  if (fileNames.has("requirements.txt") || fileNames.has("setup.py") || fileNames.has("pyproject.toml")) stack.push({ name: "Python", category: "language" });
  if (fileNames.has("go.mod")) stack.push({ name: "Go", category: "language" });
  if (fileNames.has("cargo.toml")) stack.push({ name: "Rust", category: "language" });
  if (fileNames.has("gemfile")) stack.push({ name: "Ruby", category: "language" });
  if (fileNames.has("pom.xml") || fileNames.has("build.gradle") || fileNames.has("build.gradle.kts")) stack.push({ name: "Java", category: "language" });
  if (fileNames.has("dockerfile") || fileNames.has("docker-compose.yml") || fileNames.has("docker-compose.yaml")) stack.push({ name: "Docker", category: "tool" });
  if (fileNames.has(".github")) stack.push({ name: "GitHub Actions", category: "tool" });
  if (fileNames.has("vercel.json")) stack.push({ name: "Vercel", category: "cloud" });
  if (fileNames.has(".eslintrc.js") || fileNames.has(".eslintrc.json") || fileNames.has("eslint.config.js") || fileNames.has("eslint.config.mjs")) stack.push({ name: "ESLint", category: "tool" });
  if (fileNames.has("jest.config.js") || fileNames.has("jest.config.ts")) stack.push({ name: "Jest", category: "tool" });
  if (fileNames.has("prisma")) stack.push({ name: "Prisma", category: "database" });
  if (fileNames.has(".env") || fileNames.has(".env.example")) stack.push({ name: "dotenv", category: "tool" });
...
export function calculateScore(
  ...
  return {
    total,
    breakdown: { stars, contributors: contribScore, commitFrequency, hasReadme: readmeScore, issueRatio, documentation: docScore },
    label,
  };
}
...
export function analyzeHealth(
  ...
  return {
    status, activityLevel, issueResolution, maintenance,
    lastCommitDaysAgo, openIssues: issueCount, totalCommitsRecent, contributorActivity,
  };
}

export function parseReadme(raw: string): ReadmeSummary {
  ...
  for (const line of lines) {
    ...
    if (lower.match(/^#{1,3}\s*(install|getting\s*started|setup|quick\s*start)/)) { currentSection = "installation"; continue; }
    if (lower.match(/^#{1,3}\s*(usage|how\s*to\s*use|examples?)/)) { currentSection = "usage"; continue; }
    if (lower.match(/^#{1,3}\s/) && currentSection !== "description") { currentSection = "other"; continue; }

    if (currentSection === "description" && descLines.length < 15) descLines.push(line);
    else if (currentSection === "installation" && installLines.length < 20) installLines.push(line);
    else if (currentSection === "usage" && usageLines.length < 20) usageLines.push(line);
  }
...
export function generateHiringInsights(
  ...
  if (totalRepoCount > 0 && forkCount / totalRepoCount > 0.7) redFlags.push("Over 70% of repositories are forks — limited original work");
  if (activeMonths === 0 && accountYears >= 1) redFlags.push("No activity in the last 6 months — account may be inactive");
  const emptyRepos = repos.filter(r => !r.language && !r.description);
  if (repos.length >= 3 && emptyRepos.length / repos.length > 0.5) redFlags.push("Many repositories appear empty or uninitialized");
  ...
  if (recentSum > 10 && olderSum <= 2 && repos.length > 5) redFlags.push("Sudden spike in recent activity after long inactivity — possible resume padding");
  if (repos.length === 0 && accountYears >= 2) redFlags.push("Account exists for " + accountYears + " years but has no original repositories");
  const archivedCount = repos.filter(r => r.archived).length;
  if (repos.length >= 5 && archivedCount / repos.length > 0.6) redFlags.push("Majority of repositories are archived — may indicate abandoned projects");
  ...
  if (topLanguages.length > 0) summary += `Primary skills include ${topLanguages.slice(0, 3).join(", ")}. `;
  if (repos.length > 0) {
  ...
  if (recommendation === "Strongly Recommend" || recommendation === "Recommend") summary += "Overall profile suggests a capable candidate worth considering.";
  else if (recommendation === "Consider") summary += "Profile shows potential but may benefit from further evaluation.";
  else summary += "Limited public portfolio — recommend reviewing additional qualifications.";

  const improvementTips: string[] = [];
  if (repos.length < 3) improvementTips.push("Create more public repositories to showcase your skills — aim for at least 5 quality projects");
  if (totalStars === 0 && repos.length > 0) improvementTips.push("Share your projects on social media, dev communities, or Reddit to gain visibility and stars");
  if (topLanguages.length <= 1) improvementTips.push("Try building projects in a second language to show versatility — pick one that complements your primary stack");
  if (activeMonths <= 1) improvementTips.push("Commit regularly — even small updates show consistency. Aim for at least weekly activity");
  if (descRatio < 0.3 && repos.length >= 3) improvementTips.push("Add clear descriptions and README files to all your repositories — recruiters check these first");
  if (user.followers === 0) improvementTips.push("Engage with the community — follow other developers, contribute to discussions, and open issues on projects you use");
  if (!user.bio) improvementTips.push("Add a professional bio to your GitHub profile — mention your skills, interests, and what you're working on");
  if (!user.location) improvementTips.push("Add your location to your profile — it helps recruiters looking for developers in specific regions");
  if (forkCount > 0 && repos.length < 5) improvementTips.push("Focus on creating original projects rather than forking — original work stands out more to recruiters");
  const noReadmeRepos = repos.filter(r => !r.description && r.stargazers_count === 0);
  if (noReadmeRepos.length > 0) improvementTips.push("Clean up or archive old/incomplete repos — a curated portfolio makes a stronger impression than quantity");
  if (pinnedRepoNames.length === 0 && repos.length >= 3) improvementTips.push("Pin your best repositories on your GitHub profile — these are the first things visitors see");
  if (topLanguages.length >= 2 && !user.blog) improvementTips.push("Add a portfolio website or blog link to your profile to show off your work beyond code");
...
export function buildSkillDomains(languages: LanguageBreakdown): SkillDomain[] {
...
export async function fetchContributionHeatmap(username: string): Promise<ContributionDay[]> {
  try {
